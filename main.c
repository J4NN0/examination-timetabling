...
void
swap_freetimeslots(Solution *sol, Solution **psol, int **conflicts, int *timeslots, int nexams, int nstudents, int tmax,
                   int freets);

void disp(int pos, int *valfreets, int **conflicts, Solution *sol, Solution **psol, int *mark, int *timeslots,
          int *freetimeslots, int nexams, int nstudents, int tmax, int freets);
...
void check_best_sol_no_timeslot(Solution *sol, Solution **psol, int **conflicts, int nstudents, int tmax);
...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < 100; ++i) {
        ...
        check_best_sol_no_timeslot(sol, psol, conflicts, nstudents, tmax);
    }
...
void feasible_search(Solution *sol, Solution **psol, int **conflicts, int nexams, int nstudents, int tmax) {
    ...
    priority = calloc(nexams,
                      sizeof(int)); // each indexes represent an exam and the value of that index represent the total number of conflicts between this exam and the others
    stexams = malloc(nexams *
                     sizeof(int)); // it will contain a sorted number of indexes (each index represent an exam) based on the priority of the previous array
...
void exam_local_search(Solution *sol, Solution **psol, int **conflicts, int nexams, int nstudents, int tmax) {
    ...
    for (j = 0; j < 10; ++j) {
        ...
        if (slotsToAvoid[randSlot] == 0) {
            if (sol[randSlot].currpos >=
                sol[randSlot].dim) { // i need to realloc sol[tmax].e, because of it was allocate for 'nexams'
    ...
    int examIndex;
    for (k = 0; k < sol[slot].currpos; ++k) {
        if (sol[slot].e[k] == examId) {
            sol[slot].e[k] = -1;
            examIndex = k;
        }
    }
    sol[slot].e[examIndex] = sol[slot].e[(sol[j].currpos--) - 1];
}
...
void graph_coloring_greedy(Solution *sol, int **conflicts, int *stexams, int nexams, int tmax) {
    ...
    for (i = 1; i < nexams; i++) { // stexams[i] is the exam i'm considering (priority array)
        for (j = 0; j < nexams; j++) { // i search if my exam has conflict with other exams
            if (conflicts[stexams[i]][j] > 0 &&
                colors[j] >= 0) // if there is a conflict and this exam 'j' has a color, so colors[j]!=-1
        ...
        if (flag ==
            0) { // if i can't find a color for that exam i put it in my extra time slot and i'll schedule it after
    ...
    for (i = 0; i < sol[tmax].currpos; i++) { // i use this cycle to take exams i put in the extra time slot
        ...
        for (j = 0; j < tmax; j++) { // 'j' is the hypothetical time slot in which i want to put my exam sol[tmax].e[i]
            if (tabulist[sol[tmax].e[i]][j] == 0) {
                obj_tmp = confl_exams(sol, sol[tmax].e[i], j, conflicts, tmax,
                                      nstudents); // i calculate for that time slot the temporary 'obj'
                if (obj_tmp <
                    obj_min) { // if this obj is better than the previous i remember the time slot in which my 'obj' it the best
                    obj_min = obj_tmp;
                    ts = j;
                }
            }
        ...
        if (ts == -1) // one exam has full his 'tabulist'
            return 1;
        schedule_exam(sol, sol[tmax].e[i], ts, conflicts, tabulist, nexams,
                      tmax); // i schedule my exam in time slot 'ts' which has the best temporary 'obj'
    }
...
int schedule_exam(Solution *sol, int exam, int ts, int **conflicts, int **tabulist, int nexams, int tmax) {
    ...
    for (i = 0;
         i < sol[ts].currpos; i++) { // searching for conflicting exams with exam i want to schedule in time slot 'ts'
        if (conflicts[exam][sol[ts].e[i]] > 0) {
            sol[tmax].e[sol[tmax].currpos] = sol[ts].e[i]; // put conflicting exam in the extra time slot
            sol[tmax].currpos++;
            if (sol[tmax].currpos >=
                sol[tmax].dim) { // i need to realloc sol[tmax].e, because of it was allocate for 'nexams'
    ...
    if (nconfl > 0) {
        for (i = 0; i < sol[ts].currpos && flag == 0; i++) {
            if (sol[ts].e[sol[ts].currpos - 1 - i] !=
                -1) { // from the last value of sol[ts].e[last] search for a not delete exam
                sol[ts].e[idel[j]] = sol[ts].e[sol[ts].currpos - 1 -
                                               i]; // put it in the first position contains delete exam
                j++;
...
void dens(Solution *sol, Solution **psol, int **conflicts, int nexams, int nstudents, int tmax) {
    ...
    idcorr = malloc((tmax - freets) *
                    sizeof(int)); // it will contain the index corresponding to the value of: newdens[i]=value <-> idcorr[i]=idexams
...
void density_gen(int *density, int **conflicts, Solution *sol, int tmax) {
    ...
    for (i = 0; i < tmax; i++) { //cycle in time slot sol[0, ..., tmax]
        for (k = 0; k < sol[i].currpos; k++) { //cycle in sol[i].e[0, ..., k] - exams i scheduled in time slot 'i'
            for (j = 0; j <= i + 5 && j <
                                      tmax; j++) { //after 5 i don't pay no kind of penalty - cycle to compare time slot 'i' with time slot 'j'
                for (t = 0; t <
                            sol[j].currpos; t++) { //i have to compare all exams, for this reason 'j' and 't' start from zero
...
swap_freetimeslots(Solution *sol, Solution **psol, int **conflicts, int *timeslots, int nexams, int nstudents, int tmax,
    ...
    for (i = 0; i <
                tmax; i++) // initialization of 'valfreets', i'll put a disposition of these values in 'freetimeslots' that says me in which slot i have to put the free time slot
        valfreets[i] = i;
...
void disp(int pos, int *valfreets, int **conflicts, Solution *sol, Solution **psol, int *mark, int *timeslots,
    ...
    for (i = 0; i < tmax; i++) {
        if (mark[i] == 0) {
            ...
            disp(pos + 1, valfreets, conflicts, sol, psol, mark, timeslots, freetimeslots, nexams, nstudents, tmax,
                 freets);
            mark[i] = 0;
        }
    }
